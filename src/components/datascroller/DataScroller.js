...
export class DataScroller extends Component {
  ...
  renderElements = () => {
    ...
    const { value, lazy, itemTemplate } = this.props;
    if (lazy && value) {
      return value.map((val, i) => (
        <li key={i + '_datascrollitem'}>
          {itemTemplate ? itemTemplate(val) : val}
        </li>
      ));
    }
    return (
      dataToRender &&
      dataToRender.map((val, i) => {
        var listItemContent = itemTemplate ? itemTemplate(val) : val;
        return <li key={i + '_datascrollitem'}>{listItemContent}</li>;
      })
    );
  };
  ...
  render() {
    ...
    var header = this.props.header && (
        <div className='p-datascroller-header'> {this.props.header}</div>
      ),
      footer = this.props.footer && (
        <div className='p-datascroller-footer'> {this.props.footer} </div>
      ),
      content = (
        <div
          ref={el => (this.contentElement = ReactDOM.findDOMNode(el))}
          className='p-datascroller-content'
          style={{ maxHeight: this.props.scrollHeight }}
        >
          <ul className='p-datascroller-list'>{this.renderElements()}</ul>
        </div>
      );
